		...
		if($this->attackDelay > 30 && mt_rand(1, 32) < 4 && $this->distanceSquared($player) <= 55){
			...
			$nbt = new Compound("", [
				"Pos" => new Enum("Pos", [
					...
					new DoubleTag(1, $this->y + 1.62),
					new DoubleTag(2, $this->z +(cos($yaw / 180 * M_PI) * cos($pitch / 180 * M_PI) * 0.5))
				]),
				"Motion" => new Enum("Motion", [
					new DoubleTag(0, -sin($yaw / 180 * M_PI) * cos($pitch / 180 * M_PI) * $f),
					new DoubleTag(1, -sin($pitch / 180 * M_PI) * $f),
					new DoubleTag(2, cos($yaw / 180 * M_PI) * cos($pitch / 180 * M_PI) * $f)
				]),
