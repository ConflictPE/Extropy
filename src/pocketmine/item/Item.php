...
class Item implements ItemIds, \JsonSerializable {
	...
	private static function parseCompound(string $tag) : Compound {
		if($tag === "") {
	...
	private static function writeCompound(Compound $tag) : string {
		if(self::$cachedParser === null) {
			self::$cachedParser = new NBT(NBT::LITTLE_ENDIAN);
		}
	...
	public function hasCustomBlockData() : bool {
		...
		return isset($tag->BlockEntityTag) and $tag->BlockEntityTag instanceof Compound;
	}
	...
	public function hasEnchantments() : bool {
		...
		$tag = $this->getNamedTag();
		return isset($tag->ench) and $tag->ench instanceof Enum;
	}
	...
	public function addEnchantment(Enchantment $ench) {
		...
		if(!$found) {
			$tag->ench->{count($tag->ench)} = new Compound("", [
				new ShortTag("id", $ench->getId()),
				new ShortTag("lvl", $ench->getLevel()),
			]);
		}
	...
	public function setLore(array $lines) {
		...
		$tag->display->Lore = new Enum("Lore");
	...
	/**
	 * Sets the Item's NBT from the supplied Compound object.
	 *
	 * @param Compound $tag
	 *
	 * @return $this
	...
	/**
	 * Called when a player attacks an entity using the item
	 *
	 * @param Player $player
	 * @param Entity $target
	 *
	...
	/**
	 * Called when a player interacts with an entity using the item
	 *
	 * @param Player $player
	 * @param Entity $target
	 *
	...
	/**
	 * Called when a player looks at/hovers over an entity whilst holding the item
	 *
	 * @param Player $player
	 * @param Entity $entity
	 *
	...
	final public function __toString() : string {
		return "Item " . $this->name . " (" . $this->id . ":" . ($this->hasAnyDamageValue() ? "?" : $this->meta) . ")x" . $this->count . ($this->hasCompoundTag() ? " tags:0x" . bin2hex($this->getCompound()) : "");
	}
