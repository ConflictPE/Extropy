...
class PlayerInventoryData {
	...
	public function dropItemPreprocessing() {
		...
		//$item = clone $this->cursor;
		//if($this->cursor->getId() !== Item::AIR) {
		//	var_dump("id: " . $item->getId() . " count: " . $item->getCount());
		//	$ev = new PlayerDropItemEvent($this->getHolder(), $item);
		//	$this->getHolder()->getServer()->getPluginManager()->callEvent($ev);
		//	if($ev->isCancelled()) {
		//		$this->inventory->sendContents($this->getHolder());
		//		return;
		//	}
		//	$this->inventory->remove($item);
		//	$motion = $this->getHolder()->getDirectionVector()->multiply(0.4);
		//	$this->getHolder()->level->dropItem($this->getHolder()->add(0, 1.3, 0), $item, $motion, 40);
		//}
	...
	protected function basicInventoryLogic($slot, Item $newItem, Inventory $inventory = null) {
	 ...
	 * @return null|Item
	 */
	protected function getSlotItemBasedOnTransactions(Inventory $inventory, $slot) {
		...
		while($transactionData !== false) {
			...
			if($trInventory === $inventory && $trSlot == $slot) {
				/** @todo testing */
