...
class NetworkInventoryAction {
	...
	/** @var int */
	public $windowId = ContainerIds::TYPE_NONE;
	 ...
	 * @return $this
	 */
	public function read(InventoryTransactionPacket $packet, int $playerProtocol) {
		$this->sourceType = $packet->getVarInt();

		switch($this->sourceType) {
			case self::SOURCE_CONTAINER:
				$this->windowId = $packet->getSignedVarInt();
				break;
			case self::SOURCE_WORLD:
				$this->unknown = $packet->getVarInt();
				break;
		...
		$this->inventorySlot = $packet->getVarInt();
		$this->oldItem = $packet->getSlot($playerProtocol);
		$this->newItem = $packet->getSlot($playerProtocol);
	 ...
	 * @param int $playerProtocol
	 */
	public function write(InventoryTransactionPacket $packet, int $playerProtocol) {
		...
		$packet->putVarInt($this->inventorySlot);
		$packet->putSlot($this->oldItem, $playerProtocol);
		$packet->putSlot($this->newItem, $playerProtocol);
	}
		...
		switch($this->sourceType) {
			case self::SOURCE_CONTAINER:
				if($this->windowId === ContainerIds::TYPE_ARMOR) {
					...
					$this->windowId = ContainerIds::TYPE_INVENTORY;
				}
